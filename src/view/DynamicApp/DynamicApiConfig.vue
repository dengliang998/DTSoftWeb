  ...
  <div class="crud-config-container">
    ...
    <el-card class="table-card">
      ...
      <el-row :gutter="20" style="margin-bottom: 15px;">
        <el-col :span="6">
          <el-input clearable placeholder="请输入配置名称" v-model="queryInfo.query" @clear="getDynamicApps">
            <template #append>
      ...
      <!-- 配置列表 -->
      <el-table :data="DynamicAppList" :row-style="{ height: '40px' }" :cell-style="{ padding: '0px' }" border stripe class="table-wrapper">
        ...
        <el-table-column label="操作" width="250">
          <template #default="scope">
            <div class="operation-buttons">
              ...
              <el-button type="danger" size="small" icon="Delete" @click="deleteDynamicApp(scope.row.ItemId)"></el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    >
      <el-form :model="DynamicAppForm" :rules="rules" ref="DynamicAppFormRef" label-width="100px">
        ...
        <el-form-item label="配置描述" prop="configDesc">
          <el-input type="textarea" v-model="DynamicAppForm.configDesc" placeholder="请输入配置描述" :rows="3"></el-input>
        </el-form-item>
        ...
        <div class="config-panel">
          <el-card class="panel-card">
            ...
            <div class="field-list">
              <el-button type="primary" size="small" icon="Plus" @click="addField" style="margin-bottom: 10px;">添加字段</el-button>
              <el-tree
            ...
            <div v-if="selectedFieldData" class="property-form">
              <el-form :model="selectedFieldData" label-width="120px">
                ...
                <el-form-item label="字段类型">
                  <el-select v-model="selectedFieldData.fieldType" placeholder="请选择字段类型" @change="handleFieldTypeChange">
                ...
                <!-- 下拉选择/单选/多选值配置 -->
                <el-form-item v-show="selectedFieldData.fieldType === 'select' || selectedFieldData.fieldType === 'radio' || selectedFieldData.fieldType === 'checkbox'" label="选项配置">
                    ...
                    <div v-if="selectedFieldData.options && selectedFieldData.options.length > 0">
                      <div v-for="(option, index) in selectedFieldData.options" :key="index" class="option-item">
                        ...
                        <el-input
                          ...
                          style="width: 40%; margin-right: 10px;"
                        ></el-input>
                        <el-button type="danger" size="small" icon="Delete" @click="deleteOption(index)">删除</el-button>
                      </div>
                    </div>
                ...
                <el-form-item label="字段验证规则">
                  <el-input type="textarea" v-model="selectedFieldData.validation" placeholder="请输入验证规则，JSON格式" :rows="3"></el-input>
                </el-form-item>
              ...
              <el-card v-for="api in generatedApis" :key="api.path" class="api-card">
                <template #header>
                  <div class="api-header">
                    <el-tag :type="api.method === 'GET' ? 'success' : 'primary'" style="margin-right: 10px;">{{ api.method }}</el-tag>
                    <span>{{ api.path }}</span>
    ...
    return {
      ...
      rules: {
        ...
        ModelName: [
          ...
          { pattern: /^[a-zA-Z][a-zA-Z0-9_]*$/, message: '数据模型名称只能包含英文、数字和下划线，且以英文开头', trigger: 'blur' }
        ]
      }
    };
  },
    ...
    generateApis() {
      ...
      if (SupportCreate) {
        this.generatedApis.push({
          ...
          requestParams: fields.filter(f => f.fieldName).map(field => ({
      ...
      if (SupportUpdate) {
        this.generatedApis.push({
          ...
          requestParams: [
            { name: 'id', type: 'long', required: true, description: 'ID' },
            ...fields.filter(f => f.fieldName).map(field => ({
              name: field.fieldName,
              type: field.fieldType || 'string',
              required: field.required || false,
              description: field.label || field.fieldName
