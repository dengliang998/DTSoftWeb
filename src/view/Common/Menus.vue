  ...
  <div class="menus-container">
    ...
    <el-card class="table-card">
      ...
      <el-row :gutter="20">
        <el-col :span="7">
          <!-- 搜索与添加区域 -->
          <el-input clearable placeholder="请输入菜单名称" v-model="queryInfo.query" @clear="getMenuList">
            <template #append>
    ...
    <el-dialog 
      ...
      <el-form 
        ...
        <el-form-item label="路由地址" prop="path" v-if="addForm.menuType !== 'F'">
          <el-input v-model="addForm.path" placeholder="请输入路由地址"></el-input>
        </el-form-item>
        
        <el-form-item label="组件路径" prop="component" v-show="false">
  ...
  setup() {
    ...
    const addMenu = () => {
      proxy.$refs.addFormRef.validate(async (valid) => {
        ...
        try {
          ...
          proxy.$message.error((currentAction.value === "add" ? "添加" : "修改") + "菜单失败：" + (error.response?.data?.message || error.message))
        } finally {
          loading.close()
        }
      })
    ...
    const removeMenu = async (id) => {
      try {
        ...
        if (error !== "cancel") {
          proxy.$message.error("删除菜单失败：" + (error.response?.data?.Msg || error.response?.data?.message || error.message))
        }
      }
    }
