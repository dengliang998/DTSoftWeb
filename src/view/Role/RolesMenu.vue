  ...
  <div class="role-container">
    ...
    <el-card class="table-card">
      ...
      <el-row :gutter="20">
        <el-col :span="7">
          <!-- 搜索与添加区域 -->
          <el-input clearable placeholder="请输入内容" v-model="queryInfo.query" @clear="getRoleList">
            <template #append>
      ...
      <!-- 用户列表区域 -->
      <el-table :data="RoleList" :row-style="{ height: '40px' }" :cell-style="{ padding: '0px' }"
        ...
        <el-table-column label="操作" width="230px">
          <template v-slot:default="scope">
            <div class="operation-buttons">
              <!-- 添加成员 -->
              <el-button type="primary" size="small" icon="User" :disabled="scope.row.RoleName == 'Everyone'"
                @click="showMenuPermDialog(scope.row.id, scope.row.RoleName)"></el-button>
              <!-- 配置可访问的菜单 -->
              <el-button type="primary" size="small" icon="Setting" :disabled="scope.row.RoleName == 'Administrator'"
                @click="showEditMenuDialog(scope.row.id)"></el-button>
              <!-- 修改按钮 -->
              <el-button type="primary" size="small" icon="Edit" :disabled="scope.row.RoleName == 'Administrator' || scope.row.RoleName == 'Everyone'"
                @click="showEditDialog(scope.row.id)"></el-button>
              <!-- 删除按钮 -->
              <el-button type="danger" size="small" icon="Delete" :disabled="scope.row.RoleName == 'Administrator' || scope.row.RoleName == 'Everyone'"
                @click="removeRole(scope.row.id)"></el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 修改用户的对话框 -->
    <el-dialog title="修改角色" v-model="editDialogVisible" width="50%" @close="editDialogClosed">
      <!-- 内容主体区域 -->
    ...
    <!-- 添加成员 -->
    <el-dialog title="添加成员" v-model="AddMemberDialogVisible" width="50%" @close="AddMemberDialogVisible = false">
      <!-- 内容主体区域 -->
          ...
          <el-table-column label="操作" width="180px">
            <template v-slot:default="scope">
              ...
              <el-button type="danger" size="small" icon="Delete"
                @click="removeRoleMember(scope.$index, RoleMemberList)"></el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-form>
    ...
    />
    <!-- c菜单配置 -->
    <el-dialog title="菜单权限配置" v-model="MenuDialogVisible" width="20%" @close="MenuDialogVisible = false">
      <!-- 内容主体区域 -->
      <el-form label-width="80px">
        <el-tree :props="treeprops" :load="loadNode" node-key="id" ref="tree" :default-expanded-keys="TreeExpanded"
          :default-checked-keys="TreeChecked" lazy show-checkbox default-expand-all v-if="MenuDialogVisible">
        </el-tree>
      </el-form>
      <!-- 底部区域 -->
      <template #footer>
...
export default {
  ...
  methods: {
    async getRoleList() {
      ...
      axios.post("/api/Role/GetRoleList", params)
    ...
    EditRoleMenuAuthority() {
      ...
      let MenuID = me.$refs.tree
        .getCheckedKeys()
        .concat(me.$refs.tree.getHalfCheckedKeys());
      axios
